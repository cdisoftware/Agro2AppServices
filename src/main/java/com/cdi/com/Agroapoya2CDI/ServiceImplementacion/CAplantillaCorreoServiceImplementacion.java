...
public class CAplantillaCorreoServiceImplementacion implements CAplantillaCorreoService {
    ...
    public List<CAplantillaCorreoEntity> ConsultaAplantillaCorreo(CAplantillaCorreoEntity entidad, Integer Bandera) {
        try {
            ...
            list.add(0, JSONObject.quote("ERROR LOG (paC_AplantillaCorreo)"
                    + " - Parametros: " + Bandera + "/" + entidad.getNombrePlantilla() + "/" + entidad.getIdMomentoEnvio() + "/" + entidad.getIdTipoPlantilla() + "/" + entidad.getIdPlantilla()
                    + " - ERROR JAVA = " + ex));
            return list;
        }
    }

...
import com.cdi.com.Agroapoya2CDI.Services.Distance;
...
public class KMeans {
    ...
    /**
     * Se utilizará para generar números aleatorios.
     */
    private static final Random random = new Random();

/**
     * Realiza el algoritmo de agrupación en clústeres K-Means en el conjunto de datos dado.
     *
     * param registra el conjunto de datos.
     * param k Número de clústeres.
     * param distance Para calcular la distancia entre dos elementos.
     * param maxIterations Límite superior para el número de iteraciones.
     * return K clusters junto con sus características.
     */
    public static Map<Centroid, List<Record>> fit(List<Record> records, int k, Distance distance, int maxIterations) {
        ...
        // iterar por un número predefinido de veces
        for (int i = 0; i < maxIterations; i++) {
            boolean isLastIteration = i == maxIterations - 1;

            // en cada iteración debemos encontrar el centroide más cercano para cada registro
    ...
    /**
     * Moves the given centroid to the average position of all assigned
     * features. If the centroid has no feature in its cluster, then there would
     * be no need for a relocation. Otherwise, for each entry we calculate the
     * average of all records first by summing all the entries and then dividing
     * the final summation value by the number of records.
     *
     ...
     * assignment for this centroid, first we should create the list.
     *
     * @param clusters The current cluster configuration.
    ...
    /**
     * With the help of the given distance calculator, iterates through
     * centroids and finds the nearest one to the given record.
     *
     ...
     * process, first we calculate the possible value range for each attribute.
     * Then when we're going to generate the centroids, we generate random
     * coordinates in the [min, max] range for each attribute.
     *
     * @param records The dataset which helps to calculate the [min, max] range
     * for each attribute.
